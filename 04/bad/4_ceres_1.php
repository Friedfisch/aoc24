...
class Ceres {
    ...
    public function __construct(string $search, ?int $expected = null) {
        $this->search = str_split($search);
        $this->expected = $expected;
    }

    public function loadFromFile(string $fileName = 'data4.txt'): void {
        $this->parseFileData(file($fileName));
    }

    public function parseFileData(array $data): void {
        $res = [];
    ...
    public function check(CeresDirections $case, int $row, int $col): bool {
        //print_r(func_get_args());
        //echo "found {$this->search[$charIndex]}\n";
        //if (!isset($this->data[$row][$col])) {
        //    return false;
        //}
        $charIndex = 0;
        /* $currentChar = isset($this->data[$row][$col]) ?: null;
          if ($currentChar !== $this->search[$charIndex]) {
          return false;
          }
          if (count($this->search) == $charIndex - 1) {
          echo "FOUND X:$col Y:$row I:$charIndex S:$carry D:{$case->name}\n";
          return true;
          } */
        $c = '';
        ...
        /*
          ...
          $c = $this->data[$row][$col];
          if ($c !== $this->search[$charIndex]) {
          //echo "not found {$this->search[$charIndex]}\n";
          return 0;
          }
          $carry .= $this->search[$charIndex];

          $result = false;
          foreach (CeresDirections::cases() as $case) {
          echo "X:$col Y:$row I:$charIndex C:{$this->search[$charIndex]} S:$carry D:{$case->name} \n";

          if ($row + $case->dr() < 0 || $col + $case->dc() < 0 ||
          $row + $case->dr() >= $this->height || $col + $case->dc() >= $this->width) {
          continue;
          }
          $result += $this->check($row + $case->dr(), $col + $case->dc(), $charIndex + 1, $case, $carry);
          }
          //echo "$result ";
          return $result;
         *
